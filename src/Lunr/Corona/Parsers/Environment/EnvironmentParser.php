<?php

/**
 * This file contains the request value parser for the environment name.
 *
 * SPDX-FileCopyrightText: Copyright 2025 Framna Netherlands B.V., Zwolle, The Netherlands
 * SPDX-License-Identifier: MIT
 */

namespace Lunr\Corona\Parsers\Environment;

use BackedEnum;
use Lunr\Corona\ParsedEnumValueInterface;
use Lunr\Corona\RequestEnumValueInterface;
use Lunr\Corona\RequestEnumValueParserInterface;
use Lunr\Corona\RequestValueInterface;
use RuntimeException;

/**
 * Request Value Parser for the environment name.
 */
class EnvironmentParser implements RequestEnumValueParserInterface
{

    /**
     * The parsed environment name as enum.
     * @var (BackedEnum&ParsedEnumValueInterface)|null
     */
    protected readonly ?BackedEnum $environment;

    /**
     * Whether the environment value has been initialized or not.
     * @var true
     */
    protected readonly bool $environmentInitialized;

    /**
     * The default environment name.
     * @var string
     */
    protected readonly string $defaultEnvironment;

    /**
     * The name of the environment variable holding the environment name
     * @var string
     */
    protected readonly string $envVar;

    /**
     * The name of the enum to use for client values.
     * @var class-string<BackedEnum&ParsedEnumValueInterface>
     */
    protected readonly string $enumName;

    /**
     * Constructor.
     *
     * @param class-string<BackedEnum&ParsedEnumValueInterface> $enumName           The name of the enum to use for client values.
     * @param non-empty-string                                  $defaultEnvironment Allowed API keys
     * @param non-empty-string                                  $envVar             The name of the HTTP header holding the API key.
     */
    public function __construct(string $enumName, string $defaultEnvironment, string $envVar = 'ENVIRONMENT')
    {
        $this->enumName           = $enumName;
        $this->defaultEnvironment = $defaultEnvironment;
        $this->envVar             = $envVar;
    }

    /**
     * Destructor.
     */
    public function __destruct()
    {
        // no-op
    }

    /**
     * Return the request value type the parser handles.
     *
     * @return class-string The FQDN of the type enum the parser handles
     */
    public function getRequestValueType(): string
    {
        return EnvironmentValue::class;
    }

    /**
     * Get a request value.
     *
     * @param BackedEnum&RequestValueInterface $key The identifier/name of the request value to get
     *
     * @return string|null The requested value
     */
    public function get(BackedEnum&RequestValueInterface $key): ?string
    {
        return match ($key) {
            EnvironmentValue::Environment => (isset($this->environmentInitialized) ? $this->environment : $this->parse())?->value,
            default                       => throw new RuntimeException('Unsupported request value type "' . $key::class . '"'),
        };
    }

    /**
     * Get a request value as an enum.
     *
     * @param BackedEnum&RequestEnumValueInterface $key The identifier/name of the request value to get
     *
     * @return ?BackedEnum The requested value
     */
    public function getAsEnum(BackedEnum&RequestEnumValueInterface $key): ?BackedEnum
    {
        return match ($key) {
            EnvironmentValue::Environment => isset($this->environmentInitialized) ? $this->environment : $this->parse(),
            default                       => throw new RuntimeException('Unsupported request value type "' . $key::class . '"'),
        };
    }

    /**
     * Parse the environment name from an environment variable.
     *
     * @return BackedEnum|null The parsed environment name
     */
    protected function parse(): ?BackedEnum
    {
        $this->environment = call_user_func_array([ $this->enumName, 'tryFromRequestValue' ], [ $_ENV[$this->envVar] ?? $this->defaultEnvironment ]);

        $this->environmentInitialized = TRUE;

        return $this->environment;
    }

}

?>
